...
trait ORM {
	...
	def onRead[T](block: Connection => T): T = {
	...
	def onWrite[T](block: Connection => T): T = {
	...
	def prepare[T](query: String)(block: PreparedStatement => T)(implicit conn: Connection): T = {
	...
	trait GetResult[T] extends Function1[ResultSet, T]
	...
	trait SetResult[T] extends Function2[PreparedStatement, T, Unit]
	...
	implicit class QueryExecutor(stmt: PreparedStatement) {
